...
INPUT_CSV = "My_py/光伏/数据/EnRaw/WXJY-2024-En.csv"  # 输入文件路径
OUTPUT_CSV = "My_py/光伏/数据/Processed/WXJY/WXJY-2024-Processed.csv"  # 输出文件路径
COLUMNS_TO_REMOVE = ["time1", "Cumulative total radiation", "time2", "Average total irradiance"]  # 要删除的列名列表
...
dropped_df.columns = [
    ...
    "time_windspeed", "windspeed",
    "time_envtemp", "env_temperature",
    "time_pressure", "pressure",
    "time_humidity", "humidity",
    "time_backtemp", "back_temperature",
    "time_power", "pv_power"
]

# 统一转为 datetime
time_cols = [
    "time_radiation", "time_windspeed", "time_envtemp",
    "time_pressure", "time_humidity", "time_backtemp", "time_power"
]
for c in time_cols:
...
wind_df = dropped_df[["time_windspeed", "windspeed"]].dropna().rename(columns={"time_windspeed": "time"})
...
backtemp_df = dropped_df[["time_backtemp", "back_temperature"]].dropna().rename(columns={"time_backtemp": "time"})
...
merged = merged.merge(backtemp_df, on="time", how="left")
